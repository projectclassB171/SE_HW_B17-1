# 指出一下程序的错误之处,并修改正确。
# 2k = -9     ---变量命名格式错误，不能数字开头，可改为k
#   if 2k>= 0:  ---没有遵循python代码要求的缩进格式，if前方有空格导致程序编译出现错误，将空格删除并修改2k为k
#     with = 2k +"正数"   变量命名格式错误，with为内置关键字不可以使用，with更改为with_s即可
#                        且字符串(str)与数字(int)不能使用+进行拼接，应该将k转换类型为字符串
#         print(with)    更改为print(with_s)
