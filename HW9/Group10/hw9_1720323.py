...
5. 编写一个总的入口程序，生成一个英雄， 两个低等级怪兽，一个大怪兽， 设定循环条件是英雄和怪兽按顺序轮流发送攻击，
   直到英雄死亡或所有怪兽被杀死。
6. 游戏过程中在每回合攻击、防御后应打印输出受到的伤害，以及当前的生命。游戏最后打赢英雄是Win还是Lose。
"""
import random
...
class Hero(Person):
    ...
    def upgrade(self):
        ...
        self.hp = self.max
        print('-'*10, 'level up{}级!'.format(self.level))
...
def main():
    hero = Hero('勇者', 10, 1, 'human')
    monster1 = Monster('食物')
    monster2 = Monster('工匠', 80, 1)
    boss = Boss('魔王', 100)
    ...
    if hero.get_hp() >0:
        print('game clear, {}！'.format(hero.get_name()))
    else:
        print('GG，again！')
